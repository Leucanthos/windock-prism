...
// Press F5 to snapshot all forms. Ctrl+Click or Right-click
// a control to edit; Ctrl+Right-click resize grip to resize.
// ============================================================
static class DebugOverlay
{
    ...
    static Control editTarget;
    static bool resizeMode;
    static Form activeForm;
    static bool filterAdded;

    static readonly string snapFile =
        Environment.GetEnvironmentVariable("TEMP") + @"\_debug_overlay.txt";
    ...
    class F5Filter : IMessageFilter
    {
        ...
        {
            if (m.Msg == 0x100 && (int)m.WParam == 0x74) { SnapAll(); return true; }
            return false;
        }
    }
    ...
    {
        if (activeForm != null)
            activeForm.Text = activeForm.Text.Replace(" [EDIT]", "").Replace(" [SIZE]", "");
    ...
    static void SnapAll()
    {
        System.IO.File.WriteAllText(snapFile, "");
        foreach (Form f in Application.OpenForms) LogAll(f);
    }

    static void LogAll(Form f)
    {
        System.IO.File.AppendAllText(snapFile,
