...
static class DockBar
{
    ...
    public static Form Create(){
        Icon ai=null; try{ai=new Icon(AppDomain.CurrentDomain.BaseDirectory+@"Windock.ico");}catch{}
        ...
        refreshTimer=new System.Windows.Forms.Timer{Interval=3000};refreshTimer.Tick+=(s,e)=>RefreshIcons();
    ...
    static void RefreshIcons(){
        ...
        while((hWnd=FindNextWindow(hWnd))!=IntPtr.Zero){
            ...
            try{var p=Process.GetProcessById(pid);if(p.MainWindowHandle==IntPtr.Zero)continue;
                string pn=p.ProcessName.ToLower();if(pn=="explorer"||pn=="searchapp"||pn=="textinputhost")continue;}catch{continue;}
        ...
        foreach(var di in found){
            int pid=di.Pid;
            int cnt=pwCount.ContainsKey(pid)?pwCount[pid]:1;
            di.SetBadge(cnt);
        }
        // Remove old
    ...
    public static void FocusWindow(IntPtr hWnd){if(IsIconic(hWnd))ShowWindow(hWnd,SW_RESTORE);SwitchToThisWindow(hWnd,true);}
