...
static class DebugOverlay
{
    ...
    static void MakeEditable(Control parent){
        ...
        foreach(Control c in parent.Controls){
            ...
            if(c.Width>=16&&c.Height>=16&&!(c is Label&&((Label)c).BackColor==Color.Orange)){
    ...
    static void SnapAll(){
        System.IO.File.WriteAllText(@"C:\temp\_dock_debug_overlay.txt","");
        foreach(Form f in Application.OpenForms) LogAll(f);
    }

    static void LogAll(Form f){
        System.IO.File.AppendAllText(@"C:\temp\_dock_debug_overlay.txt",
